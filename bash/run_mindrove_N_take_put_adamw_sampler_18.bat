...
REM ============================================================
REM MindRove EMG + IMU SupLoss + batch-sampler ablation for N_as_test / train_manifest_take_put.jsonl
REM 2 signals x (2 batch-sampler loss configs x 4 K_queue values + 1 random-sampler config) = 18 runs.
REM Normalization is copied from N_as_test\train_normalization_stats\take_put_mindrove_stats.json and rounded to 4 decimals.
REM ============================================================
...
set "TRAIN_MANIFEST_NAME=N_as_test\train_manifest_take_put.jsonl"
set "LABEL_MAP_JSON=%DATASET_ROOT%\label_map.json"
set "NORMALIZATION_STATS_JSON=%DATASET_ROOT%\N_as_test\train_normalization_stats\take_put_mindrove_stats.json"
set "OUT_ROOT=%PROJECT_ROOT%\results\mindrove_N_take_put_adamw_sampler_18"
...
REM ------------------------------
set "LEFT_EMG_MEAN=[0.0013, -0.0019, 0.0012, 0.0008, 0.0029, 0.0029, -0.0004, -0.0002]"
set "LEFT_EMG_STD=[15.2382, 20.2947, 22.3931, 23.2527, 23.0585, 19.6927, 10.2315, 9.3464]"
set "RIGHT_EMG_MEAN=[-0.0011, -0.0014, 0.0036, 0.0053, 0.0019, 0.0010, -0.0016, 0.0006]"
set "RIGHT_EMG_STD=[15.8905, 24.9046, 28.6934, 26.8379, 27.4055, 19.6011, 10.9768, 10.0733]"
set "LEFT_IMU_MEAN=[-0.0351, -0.0078, -0.0514, 0.1487, -0.0561, 4.4670]"
set "LEFT_IMU_STD=[0.1947, 0.1937, 0.0959, 44.1654, 48.3155, 56.6028]"
set "RIGHT_IMU_MEAN=[0.0487, -0.0318, -0.0380, -2.9322, -2.7989, -11.2903]"
set "RIGHT_IMU_STD=[0.2340, 0.2879, 0.1325, 65.6138, 76.7400, 79.8795]"
...
set "SAMPLER_TYPE_ARG=--sampler_type balanced_batch"
set "RANDOM_SAMPLER_TYPE_ARG=--sampler_type none"
set "SAMPLER_TIER_ARG="
set "BALANCED_CLASSES_PER_BATCH_ARG=--balanced_classes_per_batch 16"
set "BALANCED_SAMPLES_PER_CLASS_ARG=--balanced_samples_per_class 4"
set "BALANCED_NUM_BATCHES_ARG="
set "BALANCED_REPLACEMENT_ARG="
set "BALANCED_DROP_LAST_ARG="
...
set "PROJ_DIM=128"
set "K_QUEUE_VALUES=512 1024 2048 3840"
set "TEMPERATURE=0.1"
set "USE_MLP_ARG=--mlp"
...
echo ============================================================
echo MindRove N_as_test take_put batch-sampler ablation
...
set "RG_BALANCED_SAMPLES_PER_CLASS_ARG="
set "RG_BALANCED_NUM_BATCHES_ARG="
set "RG_BALANCED_REPLACEMENT_ARG="
set "RG_BALANCED_DROP_LAST_ARG="
if /I "!RG_SAMPLER_LABEL!"=="balanced_batch" (
    set "RG_SAMPLER_TIER_ARG=%SAMPLER_TIER_ARG%"
    set "RG_BALANCED_CLASSES_PER_BATCH_ARG=%BALANCED_CLASSES_PER_BATCH_ARG%"
    set "RG_BALANCED_SAMPLES_PER_CLASS_ARG=%BALANCED_SAMPLES_PER_CLASS_ARG%"
    set "RG_BALANCED_NUM_BATCHES_ARG=%BALANCED_NUM_BATCHES_ARG%"
    set "RG_BALANCED_REPLACEMENT_ARG=%BALANCED_REPLACEMENT_ARG%"
    set "RG_BALANCED_DROP_LAST_ARG=%BALANCED_DROP_LAST_ARG%"
)
...
if /I "!RG_SIGNAL!"=="emg" (
    "%PYTHON_BIN%" "%PY_SCRIPT%" ^
      ...
      !RG_SAMPLER_TYPE_ARG! ^
      !RG_SAMPLER_TIER_ARG! ^
      !RG_BALANCED_CLASSES_PER_BATCH_ARG! ^
      !RG_BALANCED_SAMPLES_PER_CLASS_ARG! ^
      !RG_BALANCED_NUM_BATCHES_ARG! ^
      !RG_BALANCED_REPLACEMENT_ARG! ^
      !RG_BALANCED_DROP_LAST_ARG! ^
      --ts_arch %TS_ARCH% ^
