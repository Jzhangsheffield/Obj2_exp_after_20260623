...
parser.add_argument("--backbone", default="resnet3d18", choices=SUPPORTED_BACKBONES,
                    help="RGB video backbone; all models use random initialization")
parser.add_argument("--temporal_view_mode", default="shared", choices=["shared", "independent"],
                    help="whether the two contrastive views use identical frame indices")
...
parser.add_argument("--resume_checkpoint", default=None, type=str,
                    help="resume model/optimizer/epoch from a package checkpoint")
...
def prepare_model(
    ...
    """构建 MoCo3D 模型。"""
    model = MoCo3D(
        partial(generate_video_model, backbone_name=backbone_name, model_depth=depth),
        dim=proj_dim,
        K=K,
        mlp=mlp,
        T=temp,
...
def prepare_trainloader(args, use_ddp: bool, rank: int, world_size: int):
    ...
    cfg = PackedMultiModalConfig(
        ...
        rgb_two_views=True,
        rgb_temporal_view_mode=args.temporal_view_mode,
...
def worker(args) -> None:
    ...
    try:
        ...
        if args.resume_checkpoint:
            resume_obj = torch.load(args.resume_checkpoint, map_location="cpu")
            _unwrap_model(model).load_state_dict(resume_obj["state_dict"], strict=True)
            optimizer.load_state_dict(resume_obj["optimizer"])
            args.start_epoch = max(int(args.start_epoch), int(resume_obj["epoch"]))
            log(f"[Resume] checkpoint={args.resume_checkpoint} start_epoch={args.start_epoch}")
        if is_main_process(rank):
