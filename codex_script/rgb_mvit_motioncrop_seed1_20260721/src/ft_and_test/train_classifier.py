...
from backbone.video_backbone import SUPPORTED_BACKBONES, generate_video_model
...
parser.add_argument("--backbone", default="resnet3d18", choices=SUPPORTED_BACKBONES,
                    help="RGB video backbone; this package always starts scratch models randomly")
...
parser.add_argument("--rgb_preserve_aspect_pad", action=argparse.BooleanOptionalAction, default=False,
                    help="mean-pad variable-size RGB clips to square before resizing/cropping")
...
def build_mapstyle_cfg(args, is_train: bool):
    ...
    cfg = PackedMultiModalConfig(
        ...
        rgb_preserve_aspect_pad=bool(args.rgb_preserve_aspect_pad),
...
def strip_prefixes_from_key(key: str) -> str:
    ...
    prefixes = [
        "module.",
        "model.",
        # Do not strip "backbone.": the MViT adapter intentionally stores its
        # torchvision trunk under that name.
        "encoder_q.",
        "encoder_k.",
        "encoder.",
        "base_encoder.",
...
def normalize_and_filter_state_dict(
    ...
    for k, v in raw_state_dict.items():
        outer_key = k[len("module."):] if k.startswith("module.") else k
        # Linear evaluation must use the query encoder learned by SupLoss.
        # Ignoring encoder_k also prevents its EMA weights from overwriting
        # encoder_q after prefix normalization.
        if outer_key.startswith("encoder_k."):
            dropped_missing_keys.append(outer_key)
            continue
...
def train_one_epoch(
    ...
    if args.finetune_mode == "head_only":
        # A linear probe must keep the representation fixed.  eval() freezes
        # BatchNorm running statistics and disables MViT stochastic depth;
        # the classifier itself remains trainable and is put back in train mode.
        model.eval()
        model.fc.train()
    else:
        model.train()
