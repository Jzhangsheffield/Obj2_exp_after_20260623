...
class PackedMultiModalConfig:
    ...
    rgb_temporal_view_mode: str = "independent"  # independent or shared

    # Which RGB camera field to read from the manifest.
    # 00143 -> rgb_cam_00143; 00152 -> rgb_cam_00152.
    rgb_camera_id: str = "00143"

    # Preserve the variable motion-crop aspect ratio by mean-padding to square.
    rgb_preserve_aspect_pad: bool = False
...
def resolve_rgb_manifest_key(camera_id: str) -> str:
    ...
    aliases = {
        ...
        "00143_motion_crop_m32": "rgb_cam_00143_motion_crop_m32",
        "rgb_cam_00143_motion_crop_m32": "rgb_cam_00143_motion_crop_m32",
        "00152": "rgb_cam_00152",
        "001528512812": "rgb_cam_00152",
        "cam_001528512812": "rgb_cam_00152",
        "rgb_cam_00152": "rgb_cam_00152",
        "00152_motion_crop_m32": "rgb_cam_00152_motion_crop_m32",
        "rgb_cam_00152_motion_crop_m32": "rgb_cam_00152_motion_crop_m32",
    }
    if x not in aliases:
        raise ValueError(
...
class PackedRGBDepthMapDataset(Dataset):
    ...
    def _load_rgb(self, rec: Dict[str, Any]) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        if self.cfg.rgb_two_views:
            if self.cfg.rgb_temporal_view_mode == "shared":
                idxs1 = sample_indices_strict(T, self.cfg.n_frames)
                idxs2 = list(idxs1)
            elif self.cfg.rgb_temporal_view_mode == "independent":
                idxs1, idxs2 = sample_two_views_indices(T=T, n=self.cfg.n_frames)
            else:
                raise ValueError(
                    f"Unsupported rgb_temporal_view_mode={self.cfg.rgb_temporal_view_mode!r}"
                )
...
def build_packed_mapstyle_dataset(
    ...
    if cfg.is_train:
        ...
        cfg.rgb_transform = TemporallyConsistentSpatialAugmentation(
            ...
            mean=cfg.rgb_mean,
            std=cfg.rgb_std,
            preserve_aspect_pad=cfg.rgb_preserve_aspect_pad,
        )
    else:
