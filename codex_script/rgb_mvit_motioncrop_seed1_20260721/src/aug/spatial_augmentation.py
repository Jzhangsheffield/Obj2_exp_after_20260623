...
class PadVideoToSquare:
    """Center-pad a TCHW video to square without stretching the crop."""

    def __init__(self, mean=(0.356, 0.363, 0.367)):
        self.mean = tuple(float(x) for x in mean)

    def __call__(self, video):
        x = torch.as_tensor(video)
        if x.ndim != 4 or x.shape[1] not in (1, 3):
            raise ValueError(f"PadVideoToSquare expects [T,C,H,W], got {tuple(x.shape)}")
        h, w = int(x.shape[-2]), int(x.shape[-1])
        if h == w:
            return video
        side = max(h, w)
        if x.dtype == torch.uint8:
            fill = torch.tensor([round(v * 255.0) for v in self.mean], dtype=x.dtype, device=x.device)
        else:
            fill = torch.tensor(self.mean, dtype=x.dtype, device=x.device)
        if x.shape[1] == 1:
            fill = fill[:1]
        out = fill.view(1, -1, 1, 1).expand(x.shape[0], x.shape[1], side, side).clone()
        top = (side - h) // 2
        left = (side - w) // 2
        out[:, :, top:top + h, left:left + w] = x
        return tv_tensors.Video(out) if isinstance(video, tv_tensors.Video) else out
...
class TemporallyConsistentSpatialAugmentation:
    ...
    def __init__(
        ...
        transforms = []
        if preserve_aspect_pad:
            transforms.append(PadVideoToSquare(mean=mean))
        transforms.extend([
            # v2.Resize(size=size),
