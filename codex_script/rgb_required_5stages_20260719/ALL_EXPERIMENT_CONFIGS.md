...
## 7B. Stage 4B：三 seed 与零梯度确认

Stage 4B 使用独立配置源：

```text
stage4b_small_confirmation/config/confirmation_plan.json
```

必做预训练和 full fine-tune 数组均为 `0-14,17-22`，共 21+21 个任务；`15-16` 为可选 R7。

| Family | 配置与作用 |
|---|---|
| rel_r0 | SupLoss-only relation matched control |
| rel_r9 | diff-only、K3、start125、preview EMA0.5、λrel=0.5 |
| rel_r12 | 与R9相同，但preview EMA0.9 |
| rel_null | 完全保留R9 prototype/relation路径，λrel=0 |
| proto_p0 | SupLoss-only prototype matched control |
| proto_null_p2 | `contrastive_proto`、soft、P2、start50、λproto=0 |
| proto_p4 | soft、P2、start50、λproto=0.1 |
| proto_null_p3 | `contrastive_proto`、all、P3、start50、λproto=0 |
| proto_p6 | all、P3、start50、λproto=0.1 |
| rel_r7_optional | diff-only、K10、start50、preview EMA0.5、λrel=0.5 |

Null-rel、Null-P2 和 Null-P3 不能替换为 `contrastive_only`，因为零梯度对照必须保留对应的 prototype state、refresh、EMA 和辅助损失计算路径。

增强汇总会输出三 seed 均值/标准差、逐 seed 配对差、末10 epoch稳定性、逐类别 recall、预训练辅助损失非零比例，以及最终 assignment、dead/near-dead、assignment entropy 和 prototype cosine。Stage 4B 完成前不进入 Stage 5，不运行测试集。
