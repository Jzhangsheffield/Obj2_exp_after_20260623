#!/usr/bin/env python3
"""Fine-tune one dual-camera pretraining candidate on camera 00143."""
...
def main() -> None:
    ...
    if len(matches) != 1:
        raise ValueError(f"Unknown fine-tune index: {args.index}")
    plan, task = matches[0], cfg["task"]
    ...
    cam = cfg["cameras"]["00143"]

    if save_root.is_dir() and any(save_root.rglob("last.pth")) and not args.validate_command:
        print(f"[DualCam FT Skip] completed last.pth found below {save_root}")
        return
    pretrained = None
    if plan["pretrain_id"] is not None:
        ...
        if not pretrained.is_file() and not args.dry_run and not args.validate_command:
            print(f"[DualCam FT Skip] prerequisite missing: {pretrained}")
            return
    ...
    cmd = [
        ...
        "--use_modality", "rgb", "--num_classes", str(task["num_classes"]),
        "--model_depth", "18", "--rgb_camera_id", "00143", "--rgb_size", "224",
        "--rgb_mean", *map(str, cam["mean"]), "--rgb_std", *map(str, cam["std"]),
        "--rrc_scale_min", "0.85", "--rrc_scale_max", "1.0",
        "--rrc_ratio_min", "0.9", "--rrc_ratio_max", "1.1",
        "--rgb_hflip_p", "0.5", "--rgb_vflip_p", "0.0",
        "--rgb_jitter_p", "0.0", "--rgb_gray_p", "0.0", "--rgb_blur_p", "0.0",
        "--rgb_apply_spatial_aug",
        "--epochs", "100", "--batch_size", "64",
        "--num_workers_train", "8", "--num_workers_val", "6",
        "--prefetch_factor_train", "2", "--prefetch_factor_val", "2",
        "--optimizer", "adamw", "--learning_rate", "0.001", "--weight_decay", "0.0001",
        "--schedules", "50", "75", "--seed", "1", "--finetune_mode", "full",
        "--save_period", "10", "--best_after_epoch", "0",
    ]
    if pretrained is not None:
        cmd.extend([
            ...
            "--pretrained_weight_paths", str(pretrained),
        ])
    if args.validate_command:
        cmd.insert(5, "--dualcam-parse-only")
