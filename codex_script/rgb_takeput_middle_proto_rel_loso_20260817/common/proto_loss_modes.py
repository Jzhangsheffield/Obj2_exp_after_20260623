"""Old, pre-V2 prototype-positive definitions used by this package."""
...
def prototype_contrastive_loss_soft_responsibility(
    ...
    valid = valid_label & (proto_ids.to(device=device).long() >= 0) & (
        proto_ids.to(device=device).long() < counts[safe_label]
    )
    if not bool(valid.any()):
        return q.new_zeros(())
    qv, yv = q[valid], labels[valid]
    ...
    active = (slots < counts.unsqueeze(1)).reshape(-1)
    ...
    classes = torch.arange(nclass, device=device).unsqueeze(1).expand(nclass, max_proto).reshape(-1)
    positive = active.unsqueeze(0) & classes.unsqueeze(0).eq(yv.unsqueeze(1))
    log_prob = logits - torch.logsumexp(logits, dim=1, keepdim=True)
    positive_logits = logits.masked_fill(~positive, float("-inf"))
    responsibility = torch.softmax(positive_logits, dim=1).detach()
    responsibility = torch.where(positive, responsibility, torch.zeros_like(responsibility))
    safe_log_prob = torch.where(positive, log_prob, torch.zeros_like(log_prob))
    return -(responsibility * safe_log_prob).sum(dim=1).mean()
...
def get_proto_loss(mode: str):
    ...
    if mode not in mapping:
        raise ValueError(f"Unknown prototype positive mode: {mode}")
    return mapping[mode]
