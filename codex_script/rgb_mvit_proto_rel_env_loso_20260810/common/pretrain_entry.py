#!/usr/bin/env python3
"""Run the old Proto/Rel trainer with a K400 MViT-v2-S encoder.

Patches are process-local.  Existing project files and the V2 package are never
...
'''
    ...
    warmup = int(MVIT_LR_WARMUP_EPOCHS)
    if warmup > 0 and epoch < warmup:
        ...
        for milestone in args.schedule:
            if epoch >= milestone:
                lr *= 0.1
'''
    return replace_once(source, old, new, "MViT warmup learning rate")
...
def main() -> None:
    ...
    forbidden = project / "codex_script" / "rgb_proto_rel_v2_20260804"
    ...
    if custom.parse_only:
        source = replace_once(
            ...
            'if __name__ == "__main__":\n    print("MViT old Proto/Rel command parse: OK")',
            "parse only",
        )
    ...
    if output_text and not custom.parse_only:
        ...
        (output / "mvit_old_loss_wrapper_args.json").write_text(
            json.dumps(vars(custom) | {"resume_checkpoint": resume or None}, indent=2),
            encoding="utf-8",
        )

    # Reuse the already verified Stage-6 MViT adapter, but bind it to the old
    # trainer's ResNet3D.generate_model entry point.
    ...
    factory = _make_model_factory(resnet_module, "current", "kinetics400", False)
    resnet_module.generate_model = lambda model_depth, num_classes: factory(
        "mvit_v2_s", num_classes, model_depth, False
    )
    ...
    namespace = {
        ...
        "MVIT_LR_WARMUP_EPOCHS": int(custom.lr_warmup_epochs),
